...
public class BaseFragment extends Fragment implements View.OnClickListener {
    ...
    private ActivityResultLauncher<String> mGetContent1 = registerForActivityResult(new GetContent(),
            result1 -> {
                if (result1 == null) {
                    showSnackbar1(R.string.uri_null);
                    return;
                }
                try {
                    if (isAudio1) {
                        selectAudioFileCallback1(result1);
                    } else {
                        selectTextFileCallback1(result1);
                    }
                } catch (FileNotFoundException e) {
                    e.printStackTrace();
                }
    ...
    public void selectContentModified(@NonNull String input) {
        isAudio1 = input.contains("audio");
        mGetContent1.launch(input);
    }
    ...
    public void selectTextFileCallback1(Uri result1) throws FileNotFoundException {
    }
    ...
    public void showSnackbar1(int resId) {
        Snackbar.make(requireView(), resId, Snackbar.LENGTH_SHORT)
                .setBackgroundTint(ContextCompat.getColor(requireContext(), R.color.colorPrimaryLight))
                .show();
    }
